...
from core.utils.timestamped_io import TimestampedIO
...
def update_state_with_output(
    state: ProcessState, stage: str, progress: float, status: str, output: str
):
...
def process_space(
    url: str, storage_root: str, progress_callback: Optional[ProgressCallback] = None
    ...
    try:
        ...
        def update_progress(
            stage: str, progress: float, status: str = "", output: str = ""
        ):
            ...
            try:
                    ...
                    with redirect_stdout(output_buffer), redirect_stderr(output_buffer):
                            ...
                            if d["status"] == "downloading":
                                try:
                                    ...
                                    if total > 0:
                                        ...
                                        update_progress(
                                            "download", progress * 0.8, status
                                        )

                                        # Force state save for YouTube downloads
                                        save_process_state(
                                            storage_root, space_id, state
                                        )

                                except Exception as e:
                                    logger.error(
                                        f"Error in download progress hook: {e}"
                                    )
                        ...
                        download_opts = {
                            ...
                            "outtmpl": os.path.join(
                                os.path.dirname(paths["audio_path"]), "%(id)s.%(ext)s"
                            ),
                        ...
                        try:
                            with yt_dlp.YoutubeDL(download_opts) as ydl:
                                ...
                                original_file = os.path.join(
                                    os.path.dirname(paths["audio_path"]),
                                    f"{info['id']}.{info['ext']}",
                                )
                                ...
                                if info["ext"] != "mp3":
                                    if check_ffmpeg():
                                        ...
                                        logger.warning(
                                            "ffmpeg not found - keeping original format"
                                        )
                                        update_progress(
                                            "download",
                                            1.0,
                                            "Download complete (original format)",
                                            output_buffer.getvalue(),
                                        )
                                ...
                                update_progress(
                                    "transcribe", 0.0, "Starting transcription...", ""
                                )
                            ...
                            state.update(
                                {"status": "error", "error": error_msg, "progress": 0.0}
                            )
                            save_process_state(storage_root, space_id, state)
                            raise
                        ...
                        for attempt in range(max_retries):
                            try:
                                update_progress(
                                    "transcribe",
                                    0.2,
                                    f"Transcription attempt {attempt + 1}...",
                                    output_buffer.getvalue(),
                                )
                                ...
                                if transcript:
                                    with open(
                                        paths["transcript_path"], "w", encoding="utf-8"
                                    ) as f:
                                        f.write(transcript)
                                    update_progress(
                                        "transcribe",
                                    ...
                                    if attempt < max_retries - 1:
                                        ...
                                        raise Exception(
                                            "Failed to transcribe audio after all retries"
                                        )
                ...
                if not os.path.exists(paths["quotes_path"]):
                    ...
                    with redirect_stdout(output_buffer), redirect_stderr(output_buffer):
                        try:
                            with open(
                                paths["transcript_path"], "r", encoding="utf-8"
                            ) as f:
                                transcript = f.read()
                            ...
                            temp_quotes_dir = (
                                Path(storage_root) / "temp" / space_id / "quotes"
                            )
                            ...
                            for i, chunk in enumerate(chunks):
                                if i in completed_chunks:
                                    logger.info(
                                        f"Skipping completed chunk {i+1}/{total_chunks}"
                                    )
                                    output_buffer.write(
                                ...
                                if not chunk_quotes:
                                    raise Exception(
                                        f"Failed to generate quotes for chunk {i+1}"
                                    )
                            ...
                            if all_quotes:
                                with open(
                                    paths["quotes_path"], "w", encoding="utf-8"
                                ) as f:
                                    f.write("\n\n".join(all_quotes))

                                if temp_quotes_dir.parent.exists():
                                    shutil.rmtree(
                                        str(temp_quotes_dir.parent), ignore_errors=True
                                    )
                    ...
                    with redirect_stdout(output_buffer), redirect_stderr(output_buffer):
                        try:
                            ...
                            summary = generate_summary(
                                transcript, quotes, paths["summary_path"]
                            )
                            if summary:
                                update_progress(
                                    "summary",
                ...
                state.update({"status": "error", "error": str(e), "progress": 0.0})
                save_process_state(storage_root, space_id, state)
                raise
